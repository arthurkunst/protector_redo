...
local get_radius = function(meta)

	return tostring(meta:get_int("radius") or protector_radius)
end
...
local protector_formspec = function(meta)
	...
	if i < npp then
		...
		.. (math.floor(i / 4 + 4) + 1 / 3)
		.. ";1.433,.5;protector_add_member;;]"

		-- username add button
		.."button[" .. (i % 4 * 2 + 1.25) .. ","
		.. math.floor(i / 4 + 4) .. ";.75,.5;protector_submit;+]"
...
protector.can_dig = function(r, pos, digger, onlyowner, infolevel)
	...
	local meta, owner, members, radius
	...
	for n = 1, #pos do
		...
		radius = meta:get_int("radius") or protector_radius

		if r == 0 and radius ~= r then
		...
		local p2 = minetest.find_node_near(p1, radius,
			{"protector:protect", "protector:protect2", "protector:protect_hidden"})
		
		
		
		if p2 ~= nil then
			meta2 = minetest.get_meta(p2)
			r2 = meta2:get_int("radius")
...
function minetest.is_protected(pos, digger)
	...
	if not protector.can_dig(12, pos, digger, false, 1) then
		return true
	end
...
local check_overlap = function(itemstack, placer, pointed_thing)
	...
	radius = meta:get_int("radius") or 5
	if inside_spawn(pos, protector_spawn + radius) then
	...
	if not protector.can_dig(radius + 12, pos, name, true, 3) then
...
minetest.register_node("protector:protect", {
	...
	after_place_node = function(pos, placer)
		...
		meta:set_int("radius", protector_radius)
	end,
	...
	on_rightclick = function(pos, node, clicker, itemstack)
		...
		if meta
		and protector.can_dig(0, pos, name, true, 1) then
	...
	on_punch = function(pos, node, puncher)
		...
		local display_name = get_display_name(radius)

		local display = minetest.add_entity(pos, display_name)
...
get_display_name = function(radius)
	return "protector:display_" .. radius
end

for x = 1, 12 do
	-- display entity shown when protector node is punched
	minetest.register_entity("protector:display_" .. x, {
		physical = false,
		collisionbox = {0, 0, 0, 0, 0, 0},
		visual = "wielditem",
		-- wielditem seems to be scaled to 1.5 times original node size
		visual_size = 0.67,
		textures = {"protector:display_node_" .. x},
		timer = 0,
		glow = 10,
...
for x = 1, 12 do
	minetest.register_node("protector:display_node_" .. x, {
