...
minetest.register_craftitem("protector:tool", {
	...
	on_use = function(itemstack, user, pointed_thing)
		...
		local pp = minetest.find_nodes_in_area(
			...
			{"protector:protect", "protector:protect_hidden"})

	
		local pp2 = minetest.find_nodes_in_area(
			vector.subtract(pos, 2), vector.add(pos, 2),
			{"protector:protect2"})
		...
		local radius = meta:get_string("radius") or protector_radius
		...
		-- only place on water or air
		if node.name ~= "air" and node.name ~= "default:water_source" and node.name ~= "default:river_water_source" then
			minetest.chat_send_player(name,
			S("Cannot place node, space is already occupied"))
			return 
		end
		...
		elseif type == 2 then
			if inv:contains_item("main", "protector:protect2") then

				inv:remove_item("main", "protector:protect2")

				nod = "protector:protect2"

			elseif inv:contains_item("main", "protector:protect") then
